...
def parse_args():
    ...
    parser.add_argument('-t', dest='train_data_path', type=str, required=True,
                        help='Path containing data to be trained.', default='data_csv/generated/brats+tcga4/train.csv')
    parser.add_argument('-v', dest='val_data_path', type=str, required=True,
                        help='Path containing validation data.', default='data_csv/generated/brats+tcga4/val.csv')
    parser.add_argument('-m', dest='model_file', type=str, required=True,
                        help='Name of the trained model file.', default='models/first_test.pth')
...
if __name__ == '__main__':
    ...
    #	print(item[0])
    print('Best macro-accuracy obtained:', str(best_accuracy['value']) + '% (' + str(best_accuracy['epoch']),
    ...
    with open(os.path.join('results', 'train_val', model_file.replace('.pth', '.txt')), 'w') as results_txt:
        results_txt.write('Best macro-accuracy obtained: ' + str(best_accuracy['value']) + '% (' + str(
            best_accuracy['epoch']) + ' epochs). Details (considering MICRO-accuracy):\n\n')
