...
from models import Net, select_net
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="data_csv/generated/brats+tcga4/train.csv",
    )
    parser.add_argument(
        "-v",
        dest="val_data_path",
        ...
        default="data_csv/generated/brats+tcga4/val.csv",
    )
    parser.add_argument(
        "-m",
        dest="model_file",
    ...
    parser.add_argument(
        "-lr", dest="learning_rate", type=float, default=0.001, help="Learning rate."
    )
    parser.add_argument(
        "-ep", dest="epochs", type=int, default=70, help="Number of training epochs."
    )
    parser.add_argument("-w", dest="weight_decay", type=float, default=0, help="")
    parser.add_argument(
        "-sl",
        dest="slices",
        type=int,
    ...
    parser.add_argument(
        ...
        dest="tridim",
        action="store_true",
        help="Determine that 3D convolution will be applied. If not specified, 2D convolution is "
        "applied considering slices as channels.",
    )
    parser.add_argument(
        "--no-other",
        dest="no_other",
...
if __name__ == "__main__":
    ...
    # train_set = datasets.DatasetFolder(train_data_path, load_image, extensions = ('nii.gz', 'mha'), transform =
    # augmentator())
    train_set = MedicalDataset(
        train_data_path, min_slices=n_slices, consider_other_class=consider_other_class
    )
    ...
    optimizer = optim.SGD(
        net.parameters(), lr=learning_rate, momentum=0.9, weight_decay=decay
    )
    ...
    for epoch in range(epochs):
        ...
        print(
            "Mean training loss: %.3f" % train_loss,
            "(DECREASED!)"
        ...
        with torch.no_grad():
            for pixel_data, label, _ in test_loader:
                ...
                correct_per_class[label_as_num] += (
                    (predicted == label.cuda()).sum().item()
                )
        ...
        print(
            "Validation macro-accuracy: %.3f %%" % accuracy,
            "(INCREASED!)"
        ...
        print(
            "Mean validation loss: %.3f" % (val_loss),
            "(DECREASED!)"
    ...
    # 	print(item[0])
    print(
        "Best macro-accuracy obtained:",
        str(best_accuracy["value"]) + "% (" + str(best_accuracy["epoch"]),
        "epochs). Details (considering MICRO-accuracy):",
    )
    ...
    plt.savefig(
        os.path.join("graphics", "results_" + model_file.replace(".pth", ".png"))
    )
    ...
    with open(
        os.path.join("results", "train_val", model_file.replace(".pth", ".txt")), "w"
    ) as results_txt:
        results_txt.write(
            "Best macro-accuracy obtained: "
            + str(best_accuracy["value"])
            + "% ("
            + str(best_accuracy["epoch"])
            + " epochs). Details (considering MICRO-accuracy):\n\n"
        )
